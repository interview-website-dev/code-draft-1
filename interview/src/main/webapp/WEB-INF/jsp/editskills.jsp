...
<body>
	...
	${message}
		<fieldset>
			...
			<form:form id="skillformid" modelAttribute="SkillForm"
				action="editskills?firstname=${firstname}&companyMailId=${companyemailid}"
						...
						<tr>
							...
							<td><div style="display: none;">
									...
									<option value="select a skill">select a skill</option>
										<c:forEach var="item" items="${listSkills}" varStatus="status">
						...
						<c:forEach var="skill" items="${skills}" varStatus="status">
							<tr>
								...
								<td class="col-md-4"><input
									name="skills[${status.index}].Positionid" type="hidden"
									value="${skill.getPositionid()}" /> <select
									name="skills[${status.index}].Skillname" id="skillnametag${status.index}" class="skillnameclass">
									
										<option value="${skill.getSkillname()}">${skill.getSkillname()}</option>
										<c:set var="skilln" value="${skill.getSkillname()}" />
										<c:forEach var="item" items="${listSkills}">
								...
								<td class="col-md-4"><select
									name="skills[${status.index}].Proficiencylevel" id="proficiencyleveltag${status.index}" class="proficiencylevelclass">
										...
										<c:forEach var="j" begin="1" end="3">
											<c:if test="${j ne skillp}">
												<option value="${j}">${j}</option>
											</c:if>
										</c:forEach>
								</select></td>
<td><span id="warningtag${status.index}"
						style="font-style: italic; color: red;"></span></td></td>
							</tr>
						</c:forEach>
